              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                    ...
                    {activePayload && (
                      <span
                        className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-900 border border-slate-700 text-slate-400 whitespace-nowrap shrink-0"
                        title={`This token payload contains ${Object.keys(activePayload).length} claims`}
                      >
                ...
                {(() => {
                  ...
                  if (typeof payload.iat === 'number' && typeof payload.exp === 'number' && payload.exp > payload.iat) {
                    ...
                    if (durationSec < 60) {
                      lifetimeStr = `${durationSec}s`;
                    } else if (durationSec < 3600) {
                      const m = Math.round(durationSec / 60);
                      lifetimeStr = `${m}m`;
                    } else {
                      const h = Math.round(durationSec / 3600);
                      lifetimeStr = `${h}h`;
                    }
                  }
                  if (!iatStr && !nbfStr && !expStr && !lifetimeStr) return null;
                  ...
                  return (
                    ...
                    >
                      ...
                      {lifetimeStr && (
                        <span className="flex items-center gap-1 text-slate-400" title={`Total token validity duration: ${payload.exp - payload.iat} seconds`}>
                          <Clock className="w-3 h-3 text-slate-500 shrink-0" aria-hidden="true" />
