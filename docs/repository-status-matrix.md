...
| --- | --- | --- | --- |
| JWT auth lab runtime | Beta | Go tests, Compose validation, local image build, local walkthrough automation, staged kind exercise, External Secrets-backed kind exercise, Prometheus scrape path, staged manifest validation, Go lint/fmt checks, **Rate limiting** | Managed secret injection, signing, and staged observability are implemented; now includes basic abuse controls; remaining gaps are around CI-published images and non-local staging evidence |
